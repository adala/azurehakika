    ...
    async run() {
        const configurations = [
            ...
            },
            {
                key: 'company.types',
                value: JSON.stringify([
                    { id: 1, name: 'Educational Institution', value: 'educational', is_active: true},
                    { id: 2, name: 'Corporate Organization', value: 'corporate', is_active: true},
                    { id: 3, name: 'Government Agency', value: 'government', is_active: true },
                    { id: 4, name: 'Non-Profit Organization', value: 'non_profit', is_active: true },
                    { id: 5, name: 'Recruitment Agency', value: 'recruitment', is_active: true },
                    { id: 100, name: 'Other', value: 'other',  is_active: true}
                ]),
            ...
            },
            {
                key: 'email.settings',
                value: JSON.stringify({
                    fromEmail: 'noreply@academicverify.com',
                    fromName: 'AcademicVerify',
                    supportEmail: 'support@academicverify.com'
                }),
            ...
            },
            {
                ...
                value: JSON.stringify([
                    { name: 'University of Ghana', type: 'University', code: 'UG', country: 'Ghana', is_active: true, auto: false, vfee: '15', ptime: '10 days', apiEndpint: '', apiKey: '' },
                    { name: 'Kwame Nkrumah University of Science and Technology', type: 'University', code: 'KNUST', country: 'Ghana', is_active: true, auto: false, vfee: '15', ptime: '10 days', apiEndpint: '', apiKey: '' },
                    { name: 'University of Cape Caost', type: 'University', code: 'UCC', country: 'Ghana', is_active: true, auto: false, vfee: '15', ptime: '10 days', apiEndpint: '', apiKey: '' },
                    { name: 'University of Winneba', type: 'University', code: 'UW', country: 'Ghana', is_active: true, auto: false, vfee: '15', ptime: '10 days', apiEndpint: '', apiKey: '' },
                    { name: 'University of Mines and Technology', type: 'University', code: 'UMaT', country: 'Ghana', is_active: true, auto: false, vfee: '15', ptime: '10 days', apiEndpint: '', apiKey: '' },
                ]),
                description: 'List of available institutions for verification',
                category: 'institutions'
            }
        ];

        for (const config of configurations) {
            const existing = await Configuration.findOne({ where: { key: config.key } });
            if (!existing) {
                await Configuration.create(config);
                console.log(`Created configuration: ${config.key}`);
            } else {
                // Update existing configuration
                await Configuration.update(config, { where: { key: config.key } });
                console.log(`Updated configuration: ${config.key}`);
            }
        }
...
module.exports = ConfigurationSeeder;